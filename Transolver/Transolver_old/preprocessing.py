...
def _compute_distance_to_airfoil(x: np.ndarray, idcs_airfoil: np.ndarray) -> np.ndarray:
...
def get_distance_column(sample: Dict, dist_feature_index: int = 15, recompute_if_missing: bool = True) -> np.ndarray:
    fx = sample["fx"]
...
def reindex_airfoil_after_mask(mask: np.ndarray, idcs_airfoil: np.ndarray) -> np.ndarray:
    old_to_new = -np.ones(mask.shape[0], dtype=np.int32)
    old_to_new[np.where(mask)[0]] = np.arange(mask.sum(), dtype=np.int32)
    kept_airfoil_old = idcs_airfoil[mask[idcs_airfoil]]
    return old_to_new[kept_airfoil_old].astype(np.int32)
...
def crop_sample_by_distance(sample: Dict, max_dist: float = 0.4, dist_feature_index: int = 15, recompute_if_missing: bool = True) -> Tuple[Dict, Dict]:
    d = get_distance_column(sample, dist_feature_index=dist_feature_index, recompute_if_missing=recompute_if_missing)
    mask = d < max_dist
...
def maybe_cache_preprocessed_samples(samples: List[Dict], cache_dir: Path, tag: str) -> Path:
...
def load_preprocessed_cache(path: Path):
    with open(path, "rb") as f:
        return pickle.load(f)


def preprocess_samples(samples: List[Dict], cfg, split_name: str, cache_dir: Path | None = None):
    processed = []
    stats = []
    for s in samples:
        s_out = s
        s_stats = {"n_before": int(s["x"].shape[0]), "n_after": int(s["x"].shape[0]), "retained_frac": 1.0}
        if cfg.crop_enabled:
    ...
    if cache_dir is not None and cfg.preprocess_cache_to_disk:
        cache_path = maybe_cache_preprocessed_samples(processed, cache_dir, f"{split_name}_preprocessed")
